...
class SmartyValidate {
    ...
    function is_valid(&$formvars, $form = 'default') {
        ...
        foreach($_sess as $_key => $_val) {
            ...
            if(in_array($_full_field, $_failed_fields)) {
     ...
     */
    function register_validator($id, $field, $criteria, $empty = false, $halt = false, $transform = null, $form = 'default') {
        if(!SmartyValidate::is_registered_form($form)) {
     ...
     * @param string $form name of the form (optional)
     */
    function set_transform($id, $transform, $form = 'default') {
     ...
     */
    function is_registered_validator($id, $form = 'default') {  
        if(!SmartyValidate::is_registered_form($form)) {
     ...
     */
    function unregister_validator($id, $form = 'default') {  
        if(!SmartyValidate::is_registered_form($form)) {
            return false;
        }
