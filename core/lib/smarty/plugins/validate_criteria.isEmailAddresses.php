...
function smarty_validate_criteria_isEmailAddresses($value, $empty, &$params, &$formvars) {

    if(strlen($value) == 0)
        return $empty;

    // in case value is several addresses separated by newlines
    $_addresses = explode(',', $value);
    ...
    foreach($_addresses as $_address) {
        ...
        $_is_valid = !(preg_match('!@.*@|\.\.|\,|\;!', $_address) ||
            !eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $_address));            
